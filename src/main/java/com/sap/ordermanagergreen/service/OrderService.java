...
//return resultPage;
    ...
    public Map<String, HashMap<Double, Integer>> calculate(Order order,TokenDTO token) throws CompanyNotExistException, Exception{
        ...
        Company c;
        try {
             c=companyRepository.findById(token.getCompanyId()).get();
             if(c==null)
              throw new CompanyNotExistException("company not exist");
            gate = currencyExchangeService.getGate(c.getCurrency(), order.getCurrency());
        }catch (Exception e){
